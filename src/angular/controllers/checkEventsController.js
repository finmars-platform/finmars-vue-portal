...
export default function checkEventsController($scope, $mdDialog) {
	var vm = this
	...
	vm.updateFilters = function () {
		...
		vm.eventsProjection = vm.events.filter(function (item) {
			...
			if (vm.filters.status) {
				...
				if (
					status.toLowerCase().indexOf(vm.filters.status.toLowerCase()) === -1
				) {
					result = false
				}
			}
			...
			if (vm.filters.description) {
				if (item.event_schedule_object) {
					if (
						item.event_schedule_object.display_description
							.toString()
							.toLowerCase()
							.indexOf(vm.filters.description.toLowerCase()) === -1
					) {
						result = false
					}
				} else {
			...
			if (vm.filters.instrument) {
				if (item.instrument_object) {
					if (
						item.instrument_object.user_code
							.toLowerCase()
							.indexOf(vm.filters.instrument.toLowerCase()) === -1
					) {
						result = false
					}
				} else {
				...
				if (item.position) {
					if (
						item.position
							.toString()
							.toLowerCase()
							.indexOf(vm.filters.position.toLowerCase()) === -1
					) {
						result = false
					}
				} else {
			...
			if (vm.filters.portfolio) {
				if (item.portfolio_object) {
					if (
						item.portfolio_object.user_code
							.toLowerCase()
							.indexOf(vm.filters.portfolio.toLowerCase()) === -1
					) {
						result = false
					}
				} else {
			...
			if (vm.filters.account) {
				if (item.account_object) {
					if (
						item.account_object.user_code
							.toLowerCase()
							.indexOf(vm.filters.account.toLowerCase()) === -1
					) {
						result = false
					}
				} else {
