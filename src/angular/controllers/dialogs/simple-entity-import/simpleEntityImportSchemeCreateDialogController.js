...
export default function simpleEntityImportSchemeCreateDialogController(
	$scope,
	$mdDialog,
	...
	vm.updateEntityFields = function () {
		...
		vm.scheme.entity_fields = metaService
			.getEntityAttrs(entity)
			.filter(function (item) {
				return (
					[
						'transaction_types',
						'object_permissions_user',
						'object_permissions_group',
					].indexOf(item.key) === -1 && item.value_type !== 'mc_field'
				)
			})
