...
export default function (reportHelper) {
	...
	var getInterfaceLayoutToSave = function (interfaceLayout) {
		...
		if (interfaceLayout.columnArea) {
			interfaceLayoutToSave.columnArea.height =
				interfaceLayout.columnArea.height
		}
	...
	function stashReportDates(dateFrom, dateTo) {
		...
		if (datesProps[0]) {
			// balance-report does not have dateFrom
	...
	function setObject(obj) {
		...
		if (
			...
			data.data[obj.___parentId].results &&
			data.data[obj.___parentId].results.length
		) {
			data.data[obj.___parentId].results = data.data[
				obj.___parentId
	...
	function getObject(objectId, parentId) {
		if (
			data.data[parentId] &&
			data.data[parentId].results &&
			data.data[parentId].results.length
		) {
			var result
	...
	function setActiveLayoutConfiguration(options) {
		if (options && options.layoutConfig) {
			...
			if (options) {
				if (options.isReport) {
					listLayout.data.reportOptions = metaHelper.recursiveDeepCopy(
						getReportOptions()
					)
					listLayout.data.reportLayoutOptions = metaHelper.recursiveDeepCopy(
						getReportLayoutOptions()
					)
					listLayout.data.rootGroupOptions = metaHelper.recursiveDeepCopy(
						getRootGroupOptions()
					)

					if (getExportOptions()) {
						listLayout.data.export = metaHelper.recursiveDeepCopy(
							getExportOptions()
						)
					}
	...
	function getLayoutCurrentConfiguration(isReport) {
		...
		if (isReport) {
			...
			if (
				...
				listLayout.data.reportLayoutOptions.useDateFromAbove
			) {
				listLayout.data.reportOptions = applyStashedReportDates(
					listLayout.data.reportOptions
				)
			}
	...
	function setLayoutCurrentConfiguration(
		activeListLayout,
		uiService,
		isReport
	) {
		...
		if (listLayout.data.interfaceLayout) {
			var interfaceLayout = getInterfaceLayout()
			interfaceLayout = Object.assign(
				interfaceLayout,
				listLayout.data.interfaceLayout
			)

			if (
				interfaceLayout.columnArea &&
				interfaceLayout.columnArea.height === 70
			) {
				// need for work of the old layouts
		...
		if (isReport) {
			...
			var newRootGroupOptions = Object.assign(
				{},
				rootGroupOptions,
				listLayout.data.rootGroupOptions
			)
			...
			if (viewType) {
				...
				if (
					listLayout.data.viewSettings &&
					listLayout.data.viewSettings[viewType]
				) {
			...
			if (!entityViewerOptions) {
				...
				entityViewerOptions.entity_filters = [
					'enabled',
					'disabled',
					'active',
					'inactive',
		...
		const setActiveColumnGroupSort = async (columnOrGroup) => {
			...
			var columnWithGroup = !!listLayout.data.grouping.find(
	...
	function setViewSettings(viewType, settings) {
		data.lastViewSettings[viewType] = JSON.parse(
