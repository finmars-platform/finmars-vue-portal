...
var injectEntityViewerOptions = function (entityViewerDataService) {
	...
	requestParameters.body['ev_options'] =
		entityViewerDataService.getEntityViewerOptions()
...
var injectRegularFilters = function (entityViewerDataService) {
	...
	var formatFilter = function (filter, filterType) {
		if (evRvCommonHelper.isFilterValid(filter)) {
			...
			//newRequestParametersBody = Object.assign(newRequestParametersBody, filterSettings);
			newRequestParametersBody['filter_settings'][filterType].push(
				filterSettings
			)
		}
	}
...
var deserializeObjects = function (
	...
	if (!event.___id) {
		...
		if (groupData) {
			...
			obj.___group_identifier = event.groupIdentifier
				? event.groupIdentifier
				: '-'
			// obj.___items_count = event.itemsCount ? event.itemsCount : 0;
			obj.___items_count = data.count
			obj.___is_open = true
			obj.___is_activated = evDataHelper.isGroupSelected(
				event.___id,
				event.parentGroupId,
				entityViewerDataService
			)
...
var deserializeGroups = function (
	entityViewerDataService,
	entityViewerEventService,
	data,
	requestParameters,
...
var getObjects = function (
	...
	return new Promise(function (resolve, reject) {
		...
		var pagesToRequest = requestParameters.requestedPages.filter(function (
			page
		) {
			...
			promises.push(
				new Promise(function (resolveLocal) {
					...
					if (options.groups_types) {
						options.groups_types = options.groups_types.map(function (
							groupType
						) {
					...
					evDataHelper.setDefaultObjects(
						entityViewerDataService,
						entityViewerEventService,
						requestParameters,
						pageToRequest
						...
						.then(function (data) {
							...
							if (requestParameters.loadAll) {
								if (
									...
									entityViewerDataService.setRequestParameters(
										requestParameters
									)
									...
									evDataHelper.deleteDefaultObjects(
										entityViewerDataService,
										entityViewerEventService,
										requestParameters,
										errorMessage
									...
									requestParameters.requestedPages.push(
										requestParameters.body.page
									)

									entityViewerDataService.setRequestParameters(
										requestParameters
									)
									entityViewerDataService.setActiveRequestParametersId(
				...
				new Promise(function (resolveLocal) {
					...
					groupsService
						...
						.catch(function (data) {
							...
							evDataHelper.deleteDefaultGroups(
								entityViewerDataService,
								entityViewerEventService,
								requestParameters,
								errorMessage
...
var updateDataStructure = function (
	...
	console.log(
		'updateDataStructure.requestParameters',
		JSON.parse(JSON.stringify(requestParameters))
	)
