	...
	if (obj.object_permissions) {
		...
		obj.object_permissions.forEach(function (perm) {
			if (currentMember.groups.indexOf(perm.group) !== -1) {
				...
				if (
					perm.permission === 'view_complextransaction_show_parameters' ||
					perm.permission === 'view_complextransaction_hide_parameters'
				) {
					partVisible = true
				}
...
var getAttributeTypeValue = function (obj, column) {
	...
	obj.attributes.forEach(function (item) {
		if (
			...
			item.attribute_type_object.user_code === user_code
		) {
			...
			if (column.value_type === 10 && item.value_string) {
				rawTextResult = stringHelper.parseAndInsertHyperlinks(
					item.value_string,
	...
	if (obj[column.key] != null && obj[column.key] !== undefined) {
		if (typeof obj[column.key] === 'string') {
			if (column.key === 'status') {
				...
				column.key === 'procedure_modified_datetime' ||
				column.key === 'created' ||
				column.key === 'modified'
			) {
				rawTextResult = moment(obj[column.key]).format('YYYY-MM-DD HH:mm:ss')
			} else {
				var aElem = stringHelper.parseAndInsertHyperlinks(
					obj[column.key],
					"class='openLinkInNewTab'"
				)
...
var addReconColorization = function (
	obj,
	classList,
	viewContext,
	verticalAdditions
	...
	if (
		(verticalAdditions.isOpen && verticalAdditions.type === 'reconciliation') ||
