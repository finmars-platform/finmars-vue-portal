...
export default function (
	...
	var calculateTotalHeight = function (evDataService) {
		...
		var count = evDataService.getFlatList().filter(function (item) {
			if (
				...
				item.___subtotal_type === 'proxyline'
			) {
				return false
			}
	...
	var addScrollListener = function (elements, evDataService, evEventService) {
		...
		var scrollXHandler = function () {
			if (!columnBottomRows) {
				columnBottomRows = contentWrapElem.querySelectorAll(
					'.g-column-bottom-row'
				) // Victor 2020.12.16 While old and new design work together
			}
	...
	var toggleColumnsVisibilityAfterGroupsFolding = function (
		evDataService,
		evEventService
	) {
		...
		if (columnsVisibilityChanged)
			evEventService.dispatchEvent(evEvents.COLUMNS_CHANGE)
	}

	var handleFoldButtonClick = function (
		clickData,
		evDataService,
		evEventService
	) {
		...
		if (group) {
			...
			var foldingEvent = group.___is_open
			...
			var unfoldedGroupRowsList = evDataHelper.getUnfoldedGroupsByLevel(
				group.___level,
				evDataService
			)

			if (!unfoldedGroupRowsList.length) {
				// Mark changed group and groups after it as folded
		...
		if (!lastActiveRow) {
			if (clickData.___type === 'object') {
				...
				var parent = Object.assign(
					{},
					evDataService.getData(clickData.___parentId)
				)
				var subtotal_type
				...
				if (subtotal_type === 'area') {
					parent.___is_area_subtotal_activated =
						!parent.___is_area_subtotal_activated
				}

				if (subtotal_type === 'line') {
					parent.___is_line_subtotal_activated =
						!parent.___is_line_subtotal_activated
				}
	...
	var clearContextMenuRow = function (evDataService) {
		...
		var contextMenuItem = objects.find((obj) => {
			if (obj.___type === 'group') {
				...
				if (
					group.___line_subtotal_context_menu_is_opened ||
					group.___area_subtotal_context_menu_is_opened
				) {
		...
		for (var item of flatList) {
			if (item.___type === 'subtotal') {
				if (item.___level > 0) {
					...
					var subtotalType = item.___subtotal_subtype
	...
	var handleSubtotalClick = function (
		clickData,
		evDataService,
		evEventService
	) {
		...
		var isActiveObject =
			activeObjRow &&
			activeObjRow.___id === clickData.___id &&
			activeObjRow.___parentId === clickData.___parentId
	...
	var updateDataFromCellEdit = function (
		obj,
		column,
		evDataService,
		evEventService
	...
	var getSubtotalDataFromRow = function (gRowElem) {
		...
		if (subtotalType) {
			...
			if (subtotalType === 'arealine')
				subtotalData.subType = gRowElem.dataset.subtotalSubtype
		}

		return subtotalData
	}

	var initEventDelegation = async function (
		elem,
		evDataService,
		evEventService,
		usersService,
		...
		elem.addEventListener('click', function (event) {
			...
			if (clickData.___type === 'hyperlink') {
				...
				if (cellElem) {
					...
					if (
						['instrument_principal_price', 'instrument_accrued_price'].indexOf(
							column.key
						) !== -1
					) {
						handleCellEdit(
	 ...
	 */
	var composeContextMenuForSubtotal = function (
		result,
		evDataService,
		type,
		groupId,
	...
	var checkContextMenuOptionVisibility = function (obj, option) {
		...
		if (
			obj['item_type'] === 1 &&
			obj['instrument.pricing_currency.id'] &&
			option.action === 'edit_pricing_currency'
		) {
		...
		if (
			obj['item_type'] === 1 &&
			obj['instrument.accrued_currency.id'] &&
			option.action === 'edit_accrued_currency'
		) {
			// item_type = 1 - instrument
			return true
		}
		...
		if (
			obj['item_type'] === 1 &&
			option.action === 'edit_pricing_currency_price'
		) {
			// item_type = 1 - instrument
			return true
		}
		...
		if (
			obj['item_type'] === 1 &&
			option.action === 'edit_accrued_currency_fx_rate'
		) {
			// item_type = 1 - instrument
			return true
		}

		if (obj['item_type'] === 2 && option.action === 'edit_currency') {
			// item_type = 2 - currency
			return true
		}
		...
		if (
			(obj['complex_transaction.id'] || obj['complex_transaction']) &&
			option.action === 'rebook_transaction'
		) {
			return true
		}
	...
	var composeContextMenuItem = function (
		result,
		item,
		evDataService,
		ttypes,
		...
		if (checkContextMenuOptionVisibility(obj, item)) {
			...
			if (item.action === 'book_transaction_specific') {
				...
				ttype_specific_attr =
					' data-ev-dropdown-action-data-color="' + item.action_data + '"'
			...
			if (item.action === 'open_layout') {
				result =
					result +
					'<div class="ev-dropdown-option' +
					(item.items ? ' ev-dropdown-menu-holder' : '') +
					'">' +
					'<a href="' +
					getContextMenuActionLink(evDataService, item, obj) +
					'"' +
					' target="_blank"' +
					' data-ev-dropdown-action="' +
					item.action +
					'"' +
					' data-object-id="' +
					objectId +
	...
	var generateContextMenuForSubtotal = function (
		evDataService,
		subtotalType,
		groupId,
		parentGroupHashId
		...
		result = composeContextMenuForSubtotal(
			result,
			evDataService,
			subtotalType,
			groupId,
	...
	function executeSubtotalContextMenuAction(event) {
		...
		if (dropdownAction === 'toggle_row') {
			...
			var activatedProperty =
				subtotalType === 'line'
					? '___is_line_subtotal_activated'
					: '___is_area_subtotal_activated'
			...
			if (
				activeObjRow &&
				activeObjRow.___id === subtotalId &&
				activeObjRow.___parentId === parentGroupHashId
			) {
				evDataService.setActiveObjectRow(null)
				evDataService.setActiveObject(null)
	 ...
	 */
	var addEventListenersForContextMenu = function (
		evDataService,
		evEventService,
		usersService,
		globalDataService,
	...
	var createPopupMenu = function (
		...
		if (obj) {
			...
			addEventListenersForContextMenu(
				evDataService,
				evEventService,
				usersService,
				globalDataService
	 ...
	 */
	var createPopupMenuForSubtotal = function (
		groupId,
		subtotalData,
		parentGroupHashId,
		evDataService,
		...
		var parent = evDataService.getData(parentGroupHashId)
		var type =
			subtotalData.type === 'arealine'
				? subtotalData.subType
				: subtotalData.type

		var popup = evRvDomManagerService.prepareSubtotalAndGetPopupMenu(
			groupId,
			type,
			parentGroupHashId,
			evDataService
		...
		if (parent) {
			popup.innerHTML = generateContextMenuForSubtotal(
				evDataService,
				type,
				groupId,
				parentGroupHashId
	...
	var getContextMenu = async function () {
		...
		const selectRowMenuItem = {
			// required item in first position
