<template>
	...
	>
		...
		>
			...
			<div
				@click="toggleGroupSelection($event)"
				class="ev-left-panel-tree-elem-name"
			>
				{{ getPrettyName() }} ({{ item.___items_count }})
			</div>
		...
		<div v-if="item.___is_open">
			<div v-for="child in item.results" class="ev-left-panel-children-holder">
				<g-ev-left-panel-tree-elem
					item="child"
					ev-data-service="evDataService"
					ev-event-service="evEventService"
					ev-content-element="evContentElement"
				></g-ev-left-panel-tree-elem>
			</div>
