<template>
	...
	<EvModalSaveLayoutAs v-model="openSaveAsModal" @layoutSaved="getLayouts" />

	<ModalExportListLayout
		:isReport="viewerData.isReport"
...
<script setup>
	...
	let lLayoutsLight = computed(
		() => layoutsStore.listLayoutsLightData[viewerData.content_type] || []
	)
	...
	let autosaveLayout = computed(() => {
		return lLayoutsLight.value.find(
			(lLayout) => lLayout.user_code === alUserCode
		)
	})
	...
	async function setAsDefault(layoutLight) {
		...
		if (!res.error) {
			...
			let prevDefLayout = lLayoutsLight.value.find(
				(dLayout) => dLayout.is_default
			)

			if (prevDefLayout) {
				prevDefLayout.is_default = false

				if (prevDefLayout.id === viewerData.listLayout.id)
					viewerData.listLayout.is_default = false
			}
	...
	async function renameLayout(namesData) {
		...
		if (!res.error) {
			...
			const renamedLayout = layoutsList.value.find(
				(lLayout) => lLayout.id === layout.id
			)
