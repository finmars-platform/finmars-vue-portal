...
<script setup>
	...
	const favoriteAttrs = computed(() => {
		...
		if (props.valueType) {
			...
			const noValueType = favAttrs.find(fAttr => !fAttr.value_type);

			if (noValueType) {
				console.error(`Favorite attributes for content_type '${props.content_type}' lack property 'value_type'`);
			}
			...
			return favAttrs.filter(fAttr => {
				...
				*!/
				...
				return fAttr.value_type === props.valueType;
