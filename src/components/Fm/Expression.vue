		...
		<div class="wrap pb-14 pt-5">
				...
				<div class="info ma-5">
					...
					<div v-else>
						<div class="info_item">
							...
							<div>
								parse_date - Converting string attributes to
								date attributes
							</div>
						</div>
						...
						<div class="info_item">
							...
							<code class="info_code"
								>parse_date([date_string], [format='%Y-%m-%d']):
								[date]</code
							>
							<div class="info_desc_text">
								This function may be used in: <br />
								- importing any objects (transactions, accounts,
								portfolio) with date attributes <br />
								- converting string attributes to date
								attributes
							</div>
						</div>
						...
						<div class="info_item">
							...
							<div class="info_param_block">
								...
								<div class="info_param_text">
									String with date in text format
								</div>
							</div>
							<div class="info_param_block">
								<code class="info_param">[format]</code>
								<div class="info_param_text">
									String - reflects format in which
									[date_string] is written
								</div>
							</div>
						</div>
						...
						<div class="info_item">
							...
							<div>
								parse_date(string_variable, format='%Y-%m-%d')
								for string variable with name string_variable,
								which contains this text - 2018-01-01
								parse_date(string_variable, format='%d-%m-%Y')
								for this text - 01-01-2018
								parse_date(string_variable, format='%Y/%m/%d')
								for this text - 2018/01/01
								parse_date(string_variable, format='%d.%m.%Y')
								for this text - 01.01.2018
								parse_date(string_variable, format='%Y.%m.%d')
								for this text - 2018.01.01
								parse_date('18.01.01', format='%y.%m.%d') for
								this text - 18.01.01
								parse_date('2018/September/01',
								format='%Y/%B/%d') for this text -
								2018/September/01 parse_date('01/September/18',
								format='%d/%b/%Y') for this text -
								01/September/2018
							</div>
						</div>
					</div>
			...
			<div class="right_side">
				...
				<div class="expression_function_block">
					<div v-for="(group, title) in functions" :key="title">
						<div>
							<h3 class="text-bold p-r-8 p-l-8">{{ title }}</h3>
						</div>

						<div class="fm_list">
							<div
								class="fm_list_item px-2"
								v-for="({ name, description }, i) in group"
								:key="i"
								@click="info = description"
							>
...
<script setup>
	...
	const props = defineProps(['modelValue', 'expressions']);
	...
	function change(event) {
		...
		if (event.data) {
			...
			let match = elem.innerText
				.slice(0, pos)
				.match(new RegExp('[a-z_]+' + enteredChar + '$'));

			if (match) {
				currentSnip = match;
				snippets.value = snippetsList.filter((item) =>
					item.startsWith(currentSnip)
				);
			}
		}
	...
	function paste(snippet) {
		...
		nextTick(() => {
			setCursorPosition(
				code_elem.value,
				+pos + (snippet.length - currentSnip.length) + 1
			);
		});
	}
	...
	function parse(val) {
		...
		code.value = val
			...
			.replace(/([a-zA-Z_]+)(?=\(.*\))/g, '<span class="func">$1</span>');
	}
