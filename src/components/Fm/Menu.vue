...
<script setup>
	...
	let isOpenHandler = async () => {
		...
		if (parent) {
			...
			distanceToLeft = Math.abs(
				parentRect.left - activatorRect.left - props.offsetX
			)
			distanceToRight = Math.abs(
				parentRect.right - activatorRect.right - props.offsetX
			)
			distanceToTop = Math.abs(
				activatorRect.top - parentRect.top - props.offsetY
			)
			distanceToBottom = Math.abs(
				parentRect.bottom - activatorRect.bottom - props.offsetY
			)
		} else {
			distanceToLeft = Math.abs(
				window.innerWidth - activatorRect.left - props.offsetX
			)
			distanceToRight = Math.abs(
				window.innerWidth - activatorRect.right - props.offsetX
			)
			distanceToTop = Math.abs(activatorRect.top - props.offsetY)
			distanceToBottom = Math.abs(
				window.innerHeight - activatorRect.bottom - props.offsetY
			)
		}
		...
		if (
			(((!isTop && !isBot) || isBot) &&
				(distanceToBottom >= popupRect.height ||
