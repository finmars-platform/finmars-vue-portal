			...
			<div class="e_names_popup">
				...
				<div>
					<FmInputText
						...
						@update:errorData="
							(newVal) => onErrorDataChange('public_name', newVal)
						"
					/>
				</div>
...
<script setup>
	...
	if (props.label) {
		...
		const selOpt = valueToShowOptions.find(
			(opt) => opt.id === props.valueToShow
		)
		labelText.value = selOpt.name
	}

	let notNullInputsList = Array.isArray(props.notNullInputs)
		? props.notNullInputs
		: props.notNullInputs.split(',')
	...
	function onErrorDataChange(propName, newValue) {
		...
		const inputWithError = inputsProps.find(
			(propName) => errors.value[propName]
		)
	...
	if (props.errorData)
		errors.value = JSON.parse(JSON.stringify(props.errorData))

	watch(
		() => props.errorData,
		() => {
			errors.value = props.errorData
				? JSON.parse(JSON.stringify(props.errorData))
				: {}
 ...
 */
	...
	function validateInputs() {
		notNullInputsList.forEach((propName) => {
			if (!props[propName]) {
				errors.value[propName] = {
					code: 10,
					message: 'This field should not be null',
				}
			}
		})
		...
		if (inputWithError && !errors.value.baseInput) {
			errors.value.baseInput = {
				message: 'There are fields with errors inside',
			}
		}
