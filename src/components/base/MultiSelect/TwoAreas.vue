...
<script setup>
	import {getSelectedFilter, computeSelectedList, computeAvailableList} from './multiSelectHelper';
	...
	let selectedList = computed(() => {
		...
		return computeSelectedList( selectedFilter.value, props.items, props.item_id, props.item_title )
						.filter(selItem => { // apply filter of user
	...
	let availableList = computed(() => {
		return computeAvailableList(selectedFilter.value, props.items, props.item_id, props.item_title, availableSearch.value)
	});
