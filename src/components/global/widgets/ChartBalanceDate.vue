...
<script setup>
								...
								labelsOriginal.forEach((item, i) => {
									...
									item.fillStyle =
										dashStore.instrColors[
											inputs.value.category_type +
												item.text
										];

									if (
										data.value.datasets[0].data.length <= i
									) {
										item.datasetIndex = 1;
										item.index =
											i -
	...
	async function prepareData() {
		...
		let plus = items
			...
			.map((item) => {
				plusColors.push(
					dashStore.instrColors[inputs.value.category_type + item[0]]
				);
				return item[1];
			});
		...
		let totalMinus = Math.abs(
			minus.length ? minus.reduce((a, b) => a + b) : 1
		);
