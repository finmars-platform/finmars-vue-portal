		...
		<div class="content" v-if="status === 100">
			<div class="filters flex">
				<div
					...
					@click="
						(outputs.choosed_category.__val = item), updateData()
					"
				>
					{{ item }}
				</div>
...
<script setup>
	...
	function createData() {
		...
		for (let date in historyStats[outputs.value.choosed_category.__val]) {
			...
			for (let instr in historyStats[
				outputs.value.choosed_category.__val
			][date].items) {
				if (!dataset[instr]) {
					dataset[instr] = {
						data: {},
				...
				dataset[instr].data[formatedDate] =
					historyStats[outputs.value.choosed_category.__val][
						date
					].items[instr];
				dataset[instr].total +=
					historyStats[outputs.value.choosed_category.__val][
		...
		data.datasets.forEach((item, key) => {
			item.backgroundColor =
				dashStore.instrColors[
					outputs.value.choosed_category.__val + item.label
				];

			dataOfActive.value[item.label] = item.data[activeIndex.value];
		});
		let notNull = Object.entries(
			historyStats[outputs.value.choosed_category.__val]
					...
					afterDraw: (chart, args, options) => {
						...
						metas.forEach((categoty) => {
							categoty.data.forEach((date, key) => {
								...
								if (date.height)
									allBarsByDay[key] += date.height;
							});
						});

						if (activeIndex.value == null)
							activeIndex.value = allBarsByDay.length - 1;
				...
				onClick: (evt) => {
					...
					if (points.length) {
						...
						try {
							...
							outputs.value.choosed_date.__val = Object.keys(
								historyStats[
									outputs.value.choosed_category.__val
								]
							).find((item) => item.includes(date));
