<template>
	<div ref="wrapper" class="tab-layout">
		<GridLayout
			ref="gridLayout"
		...
		>
			<template #item="{ item }">
				<div class="tab-layout__component">
					<FmIconButton
						variant="text"
						size="small"
						icon="mdi-pencil"
						color="var(--on-surface-variant)"
						class="tab-layout__component-edit"
					...
					<FmIconButton
						...
						icon="mdi-delete"
						color="var(--error)"
						class="tab-layout__component-delete"
		...
		<div class="tab-layout__actions">
			<div
				v-for="r in rowsCount"
				:key="r"
				class="tab-layout__actions-row"
			>
				<div class="tab-layout__actions-row-btn">
					<FmTooltip
						activator="parent"
						type="secondary"
						location="top"
...
<script setup>
	...
	const path = computed(() => {
		const tabIndex = (props.data?.tabs || []).findIndex(
			(tab) => tab.id === props.activeTab
		);
		return ['tabs', Math.max(tabIndex, 0)];
	});
...
<style lang="scss" scoped>
	.tab-layout {
		--tab-layout-columns-count: v-bind(columnsCount);
		--tab-layout-rows-count: v-bind(rowsCount);
		--tab-layout-row-height: v-bind(rowHeightCss);

		position: relative;
		width: 100%;
		padding-right: 72px;
		height: calc(
			calc(var(--tab-layout-row-height) * var(--tab-layout-rows-count)) +
				calc(10px * (calc(var(--tab-layout-rows-count) + 1)))
		);
		max-height: calc(
			calc(var(--tab-layout-row-height) * var(--tab-layout-rows-count)) +
				calc(10px * (calc(var(--tab-layout-rows-count) + 1)))
		...
		&__actions {
			...
			grid-template-rows: repeat(
				var(--tab-layout-rows-count),
				calc(var(--tab-layout-row-height) + 10px)
			);
	...
	.vgl-layout {
		...
		&::before {
			...
			background-size: calc(
					calc(100% - 5px) / var(--tab-layout-columns-count)
				)
				calc(var(--tab-layout-row-height) + 10px);
		}
