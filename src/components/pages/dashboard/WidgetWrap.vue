				...
				<div class="flex aic">
					<FmMenu attach="body" anchor="bottom right">
						...
						<div class="fm_list">
							<div
								class="fm_list_item"
								@click="isEditWidget = true"
							>
								Settings
							</div>
							<div
								class="fm_list_item"
								@click="remove(component.uid)"
							>
								Remove
							</div>
						</div>
...
<script setup>
	...
	function resizeX(e) {
		...
		function onmousemove(e) {
			let addedColls =
				startColls + Math.round((e.clientX - startX) / halfColl);

			component.colls =
				addedColls > component.minColls
					? addedColls
					: component.minColls;
		}
	...
	function resizeY(e) {
		...
		function onmousemove(e) {
			...
			component.rows =
				rows > component.minRows ? rows : component.minRows;
		}
	...
	function drag(e) {
		...
		document.onmouseup = function () {
			...
			let cuted = JSON.parse(
				JSON.stringify(dashStore.components[prevIndex])
			);
