<template>
	<FmCard class="realm-card">
		<div class="flex aic sb">
			...
			<div anchor="right" class="realm-right-controls-holder">
				<div
					v-if="realm.is_update_available"
					class="realm-update-is-available-chip"
				>
					Update is available
				</div>
					...
					<template #default="{ close }">
						<div class="fm_list">
							<div
								v-if="realm.is_update_available"
								class="fm_list_item"
								@click="initUpdateDatabase(), close()"
							>
							...
							<div
								class="fm_list_item"
								@click="(isProvisionLogDialog = true), close()"
							>
							...
							<div
								class="fm_list_item"
								@click="deleteRealm(), close()"
								v-if="!realm.spaces.length"
							>
							...
							<div
								class="fm_list_item"
								@click="(isAboutDialog = true), close()"
							>
	...
	<LazyPagesProfileWorkspaceProvisionLogM
		:realmCode="realm.realm_code"
		:realmId="realm.id"
		v-model="isProvisionLogDialog"
		v-if="isProvisionLogDialog"
	/>

	<LazyPagesProfileRealmAbout
		:realm="realm"
		@refresh="store.getRealms()"
