	...
	<div class="">
		...
		>
			...
			<div class="table-cell">
				{{
					Math.floor(
						(item.procedures.filter((item) => item.status == 'D')
							.length /
			...
			<div v-if="isOpen">
				...
				>
					...
					<div class="sp_item d-flex">
						...
						<div>
							{{ subprocedure.successful_prices_count }}
							successful,
							{{ subprocedure.error_prices_count }} errors
						</div>
					</div>
					<div class="sp_item d-flex">
					...
					<div class="center">
						<FmBtn
							type="action"
							@click="subprocedure.dialog = true"
						>
							Show request
						</FmBtn>

						<BaseModal
							v-model="subprocedure.dialog"
							@cancel="subprocedure.dialog = false"
							title="Procedure request"
						>
							<pre>{{
								JSON.stringify(
									subprocedure.request_data,
									null,
									2
								)
							}}</pre>
							...
							<template #controls>
								<div class="flex sb">
									<FmBtn
										type="action"
										@click="subprocedure.dialog = false"
										>cancel
									</FmBtn>
