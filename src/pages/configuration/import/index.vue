    ...
    <div v-else-if="detailsEditor.data.content" class="details-container">
      ...
      <template v-else>
        <VAceEditor
          ...
          @init="onEditorInit"
        />
        <div class="copy-wrap absolute top-4 right-4" @click="copyToBuffer(detailsEditor.data.content_formatted)">
...
<script setup>
  ...
  const progressHeightValue = computed(() => (`${heightValue.value}`));
