<template>
	<div v-if="treeLoading" class="flex-row fc-center">
		<FmLoader />
	</div>
	<div v-else>
...
<script setup>
	...
	function buildTreeData(data, nodePath, isTreeLoading = true) {
		treeLoading.value = isTreeLoading;
	...
	const listFiles = async (path) => {
		try {
			...
			if (data) {
				...
				return buildTreeData(result, path, false);
			}
			return [];
		} catch (e) {
		...
		jQuery('#jstree_explorer').on('changed.jstree', function (e, data) {
			...
			if (node) {
				...
				nodeParents.forEach((parent) => {
					if (
						parent !== '#' &&
						document.getElementById(parent + '_anchor').text === 'Root'
					) {
						return;
					}
					if (parent === '#') {
						...
						path.value =
							path.value +
							document.getElementById(parent + '_anchor').text +
							'/';
					}
				});
				if (path.value === 'Root') {
