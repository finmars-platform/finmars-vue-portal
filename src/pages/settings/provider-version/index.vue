<template>
	<div class="px-5 py-5 flex flex-col items-start justify-start gap-6">
		<div class="flex flex-col w-full gap-2">
			<span class="text-lg">Provider Version</span>
			<div
				v-if="loading"
...
<script setup>
	import ProviderVersionModal from '@/pages/settings/provider-version/provider-version-modal/index.vue';
	...
	async function editItem(item) {
		try {
			...
			await useApi('providerVersion.put', {
	...
	async function createItem(item) {
		try {
			loading.value = true;
			selectedItem.value = item;
			await useApi('providerVersion.post', {
				body: selectedItem.value
			});
			await getItemsList();
			closeModal();
	...
	async function deleteItem(item) {
		const confirm = await useConfirm({
			title: 'Delete Provider Version',
			text: `Are you sure you want delete ${item.user_code}?`
		});
		if (confirm) {
			try {
				await useApi('providerVersion.delete', {
					params: { id: item.id }
				});
				useNotify({
					type: 'success',
	...
	async function openModal(item = null) {
		...
		modalTitle.value = !item ? 'Create Provider Version' : 'Edit Provider Version';
