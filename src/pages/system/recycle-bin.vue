			...
			<div class="flex items-center mb-6">
				...
				<div class="w-[250px] mr-2">
					<ui-fm-input-date
						...
						label="Date From"
						@update:model-value="newVal => updateFiltersDebounce('date_from', newVal)"
					/>
				</div>
				...
				<div class="w-[250px] mr-2">
					<ui-fm-input-date
						...
						label="Date To"
						@update:model-value="newVal => updateFiltersDebounce('date_to', newVal)"
					/>
				</div>
				...
				<div class="flex flex-shrink basis-full mr-4 max-w-[600px]">
					<fm-text-field
						...
						outlined
						hide-details
						@update:model-value="newVal => updateFiltersDebounce('query', newVal)"
						class="mr-4"
					/>
				</div>


				<fm-button :disabled="!items.length" @click="showClearBinWarning = true">Clear Bin</fm-button>

				<base-modal v-model="showClearBinWarning" title="Warning" @close="showClearBinWarning = false">
					<div class="pb-4">
						Transactions from {{ filters.date_from }} to {{ filters.date_to }} will be <b>Deleted</b> completely.
					</div>
					...
					<template #controls="{ cancel }">
						<div class="flex justify-between items-center">
							...
							<div>
								<fm-progress-circular v-if="initializingClearing" :size="30" indeterminate class="mr-4" />

								<fm-button :disabled="initializingClearing" @click="clearBin">OK</fm-button>
								...
								<fm-tooltip
									...
									location="top"
									type="secondary"
								>Deletion will start soon</fm-tooltip>
							</div>
						</div>
					</template>
				</base-modal>
				...
				<div style="padding: 8px; padding-left: 24px;">
					<table class="angularjs-table">
						...
						<thead>
							<tr>
							<th>
								<ui-fm-checkbox :model-value="allRowsAreSelected" @update:model-value="toggleAll" class="w-max" />
							</th>
							<th>Code</th>
						...
						<tbody>
							...
							<td>
								<ui-fm-checkbox v-model="item.selected" class="w-max" />
							</td>
							<td>{{item.code}}</td>
...
<!--								<div data-ng-if="item.deleted_transaction_unique_code">
								...
								<a :href="getJournalHref(item)" target="_blank">
									<fm-button type="secondary">See Journal</fm-button>
								</a>
							</td>
						</tr>
				...
				<div>
					...
					>Restore Selected</fm-button>

					<fm-button
						type="secondary"
						:disabled="!selectedItems.length"
						@click="showDeletionWarning = true"
					>Delete Selected</fm-button>
					...
					>
						...
						<div class="pb-4">
							Transactions could be restored if <b>Unique Transaction Code</b> is free to use.<br />
							Transactions that failed restore process will stay in Recycle Bin.
						</div>
						...
						<template #controls="{ cancel }">
							<div class="flex justify-between items-center">
								...
								<div>
									<fm-progress-circular v-if="restoringInProgress" :size="30" indeterminate class="mr-4" />

									<fm-button :disabled="restoringInProgress" @click="restoreSelected">OK</fm-button>
									...
									<fm-tooltip
										...
										location="top"
										type="secondary"
									>Restoring in progress</fm-tooltip>
								</div>
							</div>
						</template>
					</base-modal>

					<base-modal v-model="showDeletionWarning" title="Warning" @close="showDeletionWarning = false">
						<div class="pb-4">
						...
						<template #controls="{ cancel }">
							<div class="flex justify-between items-center">
								...
								<div>
									<fm-progress-circular v-if="initializingDeletion" :size="30" indeterminate class="mr-4" />

									<fm-button :disabled="initializingDeletion" @click="deleteSelected">OK</fm-button>
...
async function clearBin() {
	...
	if (!res._$error) {
		...
		useNotify({type: "success", title: "Deletion of transactions started. You can see its progress inside the tasks page."});
...
function getJournalHref(item) {
	...
	queryParams = queryParams + `&query=${item.deleted_transaction_unique_code ? item.deleted_transaction_unique_code : item.code}`
...
async function fetchDeletedItems() {
	...
	res.results.forEach(item => {
		item.modified_datetime_pretty = dayjs( new Date(item.modified_at) ).format('DD-MM-YYYY HH:mm');
	});
...
async function restoreSelected() {
	...
	const res = await useApi("complexTransactionBulkRestore.post", {body: ids});
...
async function deleteSelected() {
	...
	const res = await useApi("complexTransactionBulkDelete.post", {body: data});
