...
import { getListLayoutByUserCode, getDefaultListLayout } from '~/services/uiService';

export async function getDefaultLayout(entityType) {
	const defaultLayoutData = await getDefaultListLayout(entityType);
	return get(defaultLayoutData, ['results', 0], null);
}

export async function getListLayoutByUserCode(entityType, userCode) {
	const activeLayoutData = getListLayoutByUserCode(entityType, userCode);
