...
import { getDashboardLayoutList, updateDashboardLayout } from '~/services/uiService';
import useNotify from '~/composables/useNotify';
import { md5 } from '~/utils/md5';

function filterComponentTypeByContentType(item, contentType) {
	const { type, settings } = item;
	return type === 'control' && settings.value_type === 100 && settings.content_type === contentType;
}

export const useDashboardConstructorStore = defineStore('dashboardConstructor', () => {
	...
	const componentsTypes = computed(() => layout.value.data.components_types ?? []);

	const controlComponentTypes = computed(() =>
		components.value.filter((c) => c.type === 'control')
	);
	const dateControlComponentsTypes = computed(() =>
		components.value.filter((c) => c.type === 'control' && c.settings.value_type === 40)
	);
	const currencyControlComponentsTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'currencies.currency'))
	);
	const pricingPolicyControlComponentsTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'instruments.pricingpolicy'))
	);
	const portfoliosComponentTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'portfolios.portfolio'))
	);
	const accountsComponentTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'accounts.account'))
	);
	const strategies1ComponentTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'strategies.strategy1'))
	);
	const strategies2ComponentTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'strategies.strategy2'))
	);
	const strategies3ComponentTypes = computed(() =>
		components.value.filter((c) => filterComponentTypeByContentType(c, 'strategies.strategy3'))
	);
	const componentsForLinking = computed(() => cloneDeep(COMPONENTS_FOR_LINKING));

	function getComponentsForMultiselector(userCode) {
		return components.value.reduce((res, c) => {
			const { type, user_code, id, name } = c;
			if (componentsForLinking.value.includes(type) && user_code !== userCode) {
				res.push({
	...
	function updateComponent(componentData) {
		const index = layout.value.data.components.findIndex((c) => c.id === componentData.id);
		if (index !== -1) {
	...
	function removeComponent(componentId) {
		const index = layout.value.data.components.findIndex((c) => c.id === componentId);
		index !== -1 && layout.value.data.components.splice(index, 1);

		if (!isEmpty(layout.value.data.components_types)) {
			const index1 = layout.value.data.components_types.findIndex((c) => c.id === componentId);
			index1 !== -1 && layout.value.data.components_types.splice(index1, 1);
		}
	}
	...
	async function exportComponentToDashboards(component) {
		...
		const exportedComponent = createCopyOfComponent(component, newId, namePostfix);
		...
		dialogsService.$openDialog({
			...
			dialogProps: {
				...
				cancelButton: false,
				onConfirm: async (val) => {
					const targetDashboardLayout = dashboardLayouts.filter((l) => val.includes(l.id));
					...
					try {
						const res = await Promise.all([
							targetDashboardLayout.map((l) => updateDashboardLayout(l))
						]);
